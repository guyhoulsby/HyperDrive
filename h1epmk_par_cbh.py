...
file = "h1epmk_ser_cbh"
name = "1D Linear Elastic-Plastic with Multisurface Kinematic Hardening - Parallel Bounding HARM"
mode = 0
ndim = 1
n_y = 1
const = [5.0, 4, 0.064514, 50.0, 0.216848, 30.0, 0.434694, 10.0, 0.568043, 5.0, 0.1]
mu = 0.1

def deriv():
    global E0, Einf, k, recip_k, H, R, name_const, n_int, n_inp, n_const
    Einf = float(const[0])
    n_int = int(const[1]) + 1
    n_inp = int(const[1])
    n_const = 2 + 2*n_int + 1
...
def dyda_f(chi,eps,alp):
    s = dfde(eps,alp)
    temp = np.zeros([n_y,n_int])
    temp[0,n_inp] += R*E0*hu.S(s)
    temp[0,:n_inp] += R*H*hu.S(s)
    return temp
