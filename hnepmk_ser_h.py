...
file = "hnepmk_ser_h"
name = "nD Linear Elastic - Plastic with Multisurface Kinematic Hardening - Series HARM"
mode = 1
const = [2, 100.0, 4, 0.101204, 100.0, 0.304794, 33.33333, 0.619038, 20.0, 1.021508, 10.0, 0.1]
mu = 0.1
...
def dfda(eps,alp):
    ...
    for i in range(n_int): temp[i] = -E*(eps-ep(alp))
...
def dgda(sig,alp):
    temp = np.zeros([n_int,ndim])
    for i in range(n_int): temp[i] = -sig
    temp[:n_inp] += np.einsum("n,ni->ni",H,alp[:n_inp])
    return temp
...
def dyde_f(chi,eps,alp): 
    ...
    for i in range(n_y): temp[i] = -R*E*s/t3
    return temp
