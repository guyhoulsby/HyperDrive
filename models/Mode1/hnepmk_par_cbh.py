...
file = "hnepmk_par_cbh"
name = "nD Linear Elastic-Plastic with Multisurface Kinematic Hardening - Parallel Bounding HARM"
mode = 1
const = [2, 5.0, 4, 0.064514, 50.0, 0.216848, 30.0, 0.434694, 10.0, 0.568043, 5.0, 0.1]
mu = 0.1

def deriv():
    global E0, Einf, k, recip_k, H, R, name_const
    global n_int, n_inp, n_y, n_const, ndim
    ndim = int(const[0])
    Einf = float(const[1])
    n_int = int(const[2]) + 1
    n_inp = int(const[2])
